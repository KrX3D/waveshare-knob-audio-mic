...
- add a dedicated `media_pipeline` for music streaming (start conservative: `WAV 16k mono`),
...
8. **Safe mode rollback after adding mixer/voice/media**
   - `The device reset before the boot was marked successful` means the new firmware crashed during early boot.
   - Most common causes here:
     1. Running custom `waveshare_audio`/`waveshare_mic` and built-in `speaker`/`media_player` pipeline in the same firmware (I2S contention).
     2. Memory pressure with Voice Assistant + media pipeline on S3 without PSRAM-friendly sdkconfig.
     3. Incremental config merge error (leftover blocks from previous profile).
   - Recovery sequence:
     1. Flash a minimal built-in profile only (`example_builtin_voice_assistant.yaml`).
     2. Keep mixer with separate source speakers (`spk_ann` / `spk_media`).
     3. Keep GPIO0 route-enable ON at boot.
     4. If still unstable, temporarily comment `voice_assistant:` and confirm media_player alone boots.
     5. Then re-enable `voice_assistant:` and test with `VA Start Continuous` button.


9. **Still stuck in rollback/safe-mode loop**
   - Use a staged bring-up in this exact order:
     1. Boot with only `logger`, `api`, `ota`, `output GPIO0`, and `i2s_audio + speaker` (no media_player, no voice_assistant).
     2. Add `media_player` with `announcement_pipeline` only and confirm stable boot > 60s.
     3. Add mixer + `media_pipeline` using `WAV 16k mono` first.
     4. Add `voice_assistant` last.
   - In Music Assistant, force stream/transcode to WAV/PCM 16k mono while validating.
   - If rollback continues immediately after OTA, erase the app partition once (full clean flash) and retry with staged config.


## Staged configs (when "still did not work")

If you still get rollback/safe-mode, use these files in order and only move to the next after stable boot (>60s):

1. `example_stage1_speaker_only.yaml`
   - validates DAC route + i2s speaker only.
2. `example_stage2_media_player_only.yaml`
   - adds mixer + media_player (no microphone, no voice_assistant).
3. `example_stage3_voice_assistant.yaml`
   - adds microphone + voice_assistant last.

This staged path is the fastest way to isolate which subsystem causes restart loops.
